...
def _live_engagement(now: float) -> EngagementSnapshot:
    ...
    return EngagementSnapshot(
        ...
        hyps={"proposed": 12, "testing": 4, "done": 20, "rejected": 3,
              "error": 0, "timeout": 0, "failed": 0},
        findings={"candidate": 5, "triaged": 8, "reproduced": 3, "verified": 2,
                  "exploitable": 1, "confirmed_impact": 1,
                  "false_positive": 1, "duplicate": 4, "out_of_scope": 1,
                  "wont_test": 2},
        inflight=(
        ...
        legs=(
            LegStatus(name="auditor-1", state="ok", last_round=12, verdict="CONTINUE"),
            LegStatus(name="auditor-2", state="busy", last_round=12, verdict=None),
            LegStatus(name="adjudicator", state="idle", last_round=11, verdict="CONTINUE"),
            LegStatus(name="executor", state="error", last_round=10, verdict="ROLLBACK"),
        ),
...
def _sealed_engagement(now: float, *, stale: bool,
    ...
    return EngagementSnapshot(
        ...
        hyps={"proposed": 4, "testing": 1 if canary else 0, "done": 30,
              "rejected": 2, "error": 0, "timeout": 0, "failed": 0},
        findings={"candidate": 0, "triaged": 12, "reproduced": 2, "verified": 4,
                  "exploitable": 1, "confirmed_impact": 2,
                  "false_positive": 0, "duplicate": 5, "out_of_scope": 3,
                  "wont_test": 0},
        inflight=(),
        cooldowns=(),
        wave=WaveProgress(current=2, total=2, tools_done=41, tools_total=41,
                          findings_new=9, eta_s=None),
        legs=(LegStatus(name="auditor-1", state="ok", last_round=7, verdict="STOP"),
              LegStatus(name="auditor-2", state="ok", last_round=7, verdict="STOP")),
        gates=GatesSummary(sections_ok=13, sections_total=13,
