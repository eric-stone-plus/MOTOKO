...
HealthIssue / HealthReport are deliberately generic
(severity/kind/message/evidence/suggestion + markdown + high_count) so any
other flow engine can consume the report; MOTOKO keeps this self-contained
so the engine has no cross-project import.
"""
...
def _check_orphan_assets(con, engagement_id: str, report: HealthReport) -> None:
    ...
    for r in con.execute(
        ...
        if aid:
            referenced.add(aid)
    orphans = []
    # `id` is a typed COLUMN, not a data-JSON
    # field — the old `d.get("id")` read always None, so EVERY settled
    # non-frontier asset reported as an orphan (100% false positives).
    for r in con.execute(
...
def _check_stuck_testing(con, engagement_id: str, report: HealthReport) -> None:
    ...
    This used to report the bare ``state='testing'`` count and label it a
    permanent gap (error/timeout never returns a hypothesis to proposed).
    Two things changed:
