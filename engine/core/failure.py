"""Failure taxonomy + recovery dispatch.

The engine detected failures but never acted on them. The giveaway is a
hypothesis census shaped like this:

    proposed N   testing M   error 0   timeout 0   done 0

``error``/``timeout``/``done`` are all zero, so ``orchestrator._retire``'s
``testing -> error|timeout|done`` convergence never fires — every hypothesis
that reaches ``testing`` is stranded there with no forward path.
``graph_health._check_stuck_testing`` reports exactly this and its own
suggestion string admits the gap. The engine was surfacing a bug it did not
fix.
...
to recover" — the exact failure mode that lets a graph full of stranded
hypotheses look healthy. Read-side tolerance is explicit and counted
(``FailureDigest.read_errors``) so degradation is visible.
"""
...
# advisory today — no dispatcher reads it yet; the real gates
...
def classify(*, status: str | None, exit_code: int | None = None,
    ...
    if status == "error" or (exit_code not in (0, None)):
        ...
        # sharpens the signature. Classifying a 429 as merely transient
        # swallows exactly this case.
...
def recycle_stuck_hypotheses(writer, engagement_id: str, *,
    ...
    """Close the retirement gap: a hypothesis left in ``testing`` with no run in
    ...
    for r in rows:
        ...
        # round-6 audit: deterministic NEVER_RETRY classes (scope_blocked /
        # tool_missing / spawn_error) reject IMMEDIATELY — recycling them
        # three times was pure churn; `detected` stays out (transient, the
        # cooldown owns pacing).
...
def reap_zombie_runs(writer, engagement_id: str) -> int:
    """M-4 (round-3 audit): close ``running`` tool_runs whose process is
    gone. The executor registers pids; a crash leaves rows in
    ``running`` forever, which blocks hypothesis retirement and lies in
    every digest. A row whose pid is absent from /proc is terminal:
    ``timeout`` / exit 124 (the semantics of a run we can no longer
    observe). Returns the number of rows closed. Never raises."""
    import os
    con = _conn_of(writer)
    closed = 0
    try:
