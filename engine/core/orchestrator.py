...
# Stop minting fresh hypotheses for a rule once this many of its
# hypotheses sit proposed. EXPAND_BUDGET alone does not provide this cap:
# a single cheap rule (bootstrap) can mint hundreds of hypotheses and
# starve every other category.
_MAX_PROPOSED_PER_RULE = 40

# H3 (round-3 audit): unrendered placeholder detector for the ACT-time
# fail-closed check. Only KNOWN context keys count — curl's own %-vars
# (%{http_code}, %{redirect_url}) use the same braces by design.
...
def default_fetcher(url: str, bind_ip: str | None = None):
    ...
    if not bind_ip:
        return None
    # The replay fetcher is real target traffic on a RAW socket — proxy env
    # vars cannot route it, so a direct connect during validation leaks the
    # host's own IP to the target. Fail closed unless the DEPLOYMENT itself
    # is the verified egress: MOTOKO_ALLOW_DIRECT_REPLAY=1 is an explicit
    # assertion by the operator that the host route is already anonymous.
    if not os.environ.get("MOTOKO_ALLOW_DIRECT_REPLAY"):
        return None                # fail-closed inconclusive (F26)
    u = urlparse(url)
...
class Orchestrator:
    def __init__(self, engagement_id: str, *, root=None,
        ...
        # OPSEC: per-origin cooldowns + canary accounting.
        ...
        # M-1 (round-3 audit): cooldowns survive engine restarts — load
        # the last snapshot so a crash does not silently lift a WAF block.
        try:
        ...
        # {oob} producers exist only when the engagement declares an
        # OOB domain (F26 discipline — no fabricated canary callbacks).
    ...
    def run(self, max_cycles: int = 20, max_depth: int = 3) -> dict:
        try:
            ...
            # Whatever happens inside the loop (exception, interrupt,
    ...
    def _ingest_raw(self, obs_id: str, tool: str, raw_path: str,
        ...
        # Enum tools stamp the SOURCE asset too, so the parent-domain gate
        # closes even when the tool output doesn't contain the apex host
        # (keep the self-stamp, ADD the source stamp).
                ...
                if src_aid:
                    ...
                    if src and src.get("kind") == "asset":
                        ...
                        if src_host and src.get("enumerated_host") != src_host:
                            ...
                            changed_src = True
                        # Stamp the registrable domain too — the domain
                        # gate must close even when the tool output never
                        # contains the apex host.
    ...
    def _ingest_finding(self, f: dict) -> str:
        ...
        M2/M3 (round-3 audit): a finding with NO url cannot be validated —
        it goes straight to ``wont_test`` (still on the graph as a record,
        excluded from the validator queue) and its dedup key carries
        title/detector so distinct url-less findings do not collapse. A
        finding WITH an url whose asset is missing mints a frontier asset
        (so its rule chain can fire) — but only when the scope guard
        clears the host; out-of-scope findings stay unlinked.
        """
        if not f.get("url"):
            ...
            ok, reason = self.writer.advance_and_persist(
                f["id"], "wont_test", actor="validator")
            if not ok:
    ...
    def _validate(self) -> None:
        ...
        # round-4 audit: queue hygiene — (a) validators whose backend is
        # not wired (dom without browser, oob without canary) never consume
        # slots (they can only ever return inconclusive); (b) reproduced
        # rows re-enter the queue ONLY for hard-evidence validators (a
        # replay re-check cannot promote past reproduced, so it is churn).
        def _select(f):
            name = pick_validator(f)
            if name in ("dom", "oob") and                     (name == "dom" and self.browser is None or
                     name == "oob" and self.canary is None):
                return None
            if f.get("state") == "reproduced" and name == "replay":
                return None
            return name

        selected = [(f, _select(f)) for f in findings]
        ...
        for f, name in selected[:5]:
            ...
            if verdict.event == "inconclusive":
                # round-4 audit: consecutive inconclusive verdicts mean the
                # validator can never decide for this finding — after three
                # attempts it leaves the queue (wont_test) instead of
                # churning a slot every cycle forever.
                cur = self.writer.get_entity(f["id"])
                attempts = int((cur or {}).get("validator_attempts") or 0) + 1
                if cur:
                ...
                if attempts >= 3:
                    ok, reason = self.writer.advance_and_persist(
                        f["id"], "wont_test", actor="validator",
                        verdict=Verdict("wont_test",
                                        f"inconclusive churn after "
                                        f"{attempts} attempts — exiting "
    ...
    def _run_validator(self, name: str, finding: dict):
        ...
        try:
            if name == "replay":
                # Connect to the address the guard actually cleared —
    ...
    def _expand(self) -> None:
        ...
        # Per-rule proposed backlog cap: once a rule has 40+ hypotheses
        # waiting, stop minting more for it. A single cheap rule can
        # otherwise mint hundreds and starve every other category.
        ...
        # cycle.
        # Enum mutex — one SUB/GAU hypothesis in flight per registrable
        # domain (the complement of the domain gate, applied at the mint
        # door). A sibling subdomain joining the graph no longer mints a
        # duplicate enum run for the same domain.
        _ENUM_RULES = {"R-RECON-SUB-001", "R-RECON-GAU-001"}
        enum_domains_in_flight: set[str] = set()
        for h in self.writer.query_entities(kind="hypothesis",
                                            engagement_id=self.engagement_id):
            if h.get("rule_id") in _ENUM_RULES and                     (h.get("state") or "proposed") in ("proposed", "testing")                     and h.get("domain"):
                enum_domains_in_flight.add(h["domain"])
        budget = _EXPAND_BUDGET
        # round-6 audit: most-starved assets first (ascending expansion
        # count) so a chatty host cannot monopolize the expand budget.
        ...
        for a in assets:
            ...
            if hyps:
                ...
                for hyp in hyps:
                    ...
                    if rid in _ENUM_RULES:
                        ...
                        a_domain = (util.registrable_domain(a_host)
                                    if a_host else "")
                        if a_domain and a_domain in enum_domains_in_flight:
                            dup_skips += 1
                            continue
                    # A (rule, asset) pair that
                    ...
                    if a.get("value") and a["value"].startswith(("http://", "https://")):
                        ...
                        # SUB/GAU enumerate the REGISTRABLE domain, not the
                        # subdomain itself: `subfinder -d <one host>` finds
                        # nothing, `-d <its registrable domain>` finds the
                        # whole sibling set.
                ...
                budget -= 1
                # round-6 audit: ANY non-minting pass (dup / cap-skip /
                # mixed) is an empty expansion and counts toward the
                # retirement budget — cap-skipped pairs do not come back
                # soon (backlog caps + the enum mutex), so waiting on them
                # burned idle evaluation cycles every single pass.
                if minted_any:
                    a["frontier"] = False
    ...
    def _fact_view(self, asset: dict, assets: list[dict] | None = None) -> dict:
        ...
        # Domain-level enum gate: N hosts under one registrable domain must
        # not mint N subfinder runs for that same domain.
        ...
        # round-4 audit: a host whose every known status is non-2xx (403
        # walls, auth portals) keeps its ffuf door open even when no robots
        # probe can ever return 200 — the gate opens either way.
        twoXX = any(
        ...
        # OPSEC: the robots probe stamps robots_host; fuzz and
        ...
        # surface finding classes onto facts for vuln rules — round-4
        # audit: a LIST, because several findings on one asset used to
        # overwrite each other (last-write-wins hid whole classes from the
        # vuln rules). contains-op rules see every class now.
        classes: list = []
    ...
    def _prioritize(self) -> list[dict]:
        ...
        Category quotas. With static priorities + K=1, the lowest-ranked
        category is mathematically unreachable behind a backlog of
        higher-ranked hypotheses — a run can cycle for dozens of rounds
        without ever firing one scan tool. The batch now reserves one slot
        per actionable category before filling the rest by priority.
        """
        ...
        for cat in ("vuln", "scan", "context", "tech", "injection", "access", "chain"):
            for h in hyps:
                ...
                if h.get("category") == cat:
                    ...
                    seen_ids.add(h["id"])
                    break
        # fill remaining slots by pure priority — H1 (round-3 audit): the
        # reserved-category winners are NEVER truncated; _ACT_K only caps
        # the priority-fill phase. `return batch[:_ACT_K]` used to drop up
        # to three winners when every category had a candidate, reviving
        # the exact starvation R7-6 fixed through the truncation door.
        for h in hyps:
            if h["id"] in seen_ids:
                continue
            if len(batch) >= _ACT_K:
    ...
    def _act(self, batch: list[dict]) -> None:
        ...
        for hyp in batch:
            # Opsec skips are PER-HYPOTHESIS — a batch
            # level counter let hyp A's cooldown skips retire hyp B (a
            # permanently scope-refused one) back to proposed.
            opsec_skipped = 0
            render_refused = 0
            # Mark testing BEFORE the action loop. Writing it after the
            # loop means the executor-side retirement guard
            # (state == "testing") never fires, so hypotheses stall in
            # `proposed` forever. Tests that only cover the direct-call path
            # _act never takes will stay green through that regression.
            # Retirement is AGGREGATED below, after the loop.
            if hyp.get("state") != "testing":
                hyp["state"] = "testing"
                self.writer.upsert_entity(hyp)
            started = 0          # runs actually STARTED this loop
            run_ids: list[str] = []
            for action in hyp.get("actions", []):
                ...
                # Opsec blocks are transient, so this is not terminal.
                ...
                # H3 (round-3 audit): a KNOWN context key left unrendered
                # means the engine failed to supply it ({url} on a host
                # asset, {out} without injection...) and the tool would
                # attack the literal string. Fail closed: refuse, record,
                # let the retirement treat it as a rule-authoring defect.
                unknown = [n for tok in rendered.argv
                           for n in _PLACEHOLDER_RE.findall(str(tok))
                           if n.lower() in cmd.CTX_KEYS]
                if unknown:
                    render_refused += 1
            ...
            # AGGREGATED retirement after the action loop, counting
    ...
    def _retire_hypothesis_if_complete(self, hyp: dict, *,
        ...
        Retirement is aggregated here after the whole action loop, not
        per-run in the executor. ``expected`` is
        ...
        'done' (NOT error/timeout: those re-mint and would churn forever
        against a scope that will keep refusing).

        Opsec blocks are TRANSIENT (a 30-min cooldown
        ...
        try:
            ...
            if not rows and started == 0:
                ...
                if cur and cur.get("kind") == "hypothesis" and \
                    ...
                    if render_refused:
                        # H3: a rule-authoring defect is deterministic —
                        # terminal error so it stops consuming ACT slots.
                        cur["state"] = "error"
    ...
    def _command_ctx(self, hyp: dict, action: dict) -> dict:
        ...
        """
        ctx: dict = {}
        # HIGH-4 (round-3 audit): action-level values WIN over the
        # hypothesis's (they are the more specific override).
        ...
        # H3 (round-3 audit): supply the two remaining producer keys so
        # their rules pass the placeholder fail-closed gate — {out} is a
        # per-hypothesis artifact dir (stable across the rule's actions so
        # git-dumper and trufflehog agree on the path); {oob} only when the
        # engagement actually carries an OOB domain (F26: no canary
        # backend, no fabricated callbacks).
        if "out" not in ctx:
            ...
            ctx["out"] = str(art / f"out-{hyp.get('id', 'x')}")
    ...
    def _checkpoint(self) -> None:
        self.writer.commit()
        # M-1: persist live cooldowns next to the graph.
        try:
    ...
    def close(self) -> None:
        reap = getattr(self.executor, "reap", None)
        if callable(reap):
            reap()                       # no scan outlives the engine
        self.writer.close()
