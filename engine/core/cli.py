...
def cmd_strix(args) -> int:
    ...
    strix is the autonomous pentest agent cited in INSTRUMENTS.md — this
    command is the manual/out-of-band deep-dive interface; inside the graph
    the same tool fires from a rule that gates on a deep-dive fact.
    """
    import shutil
    import subprocess
    from pathlib import Path
...
def validate_loop_config(cfg: dict) -> list[str]:
    ...
    if not auditors:
        problems.append("missing 'auditors' (need at least one auditor)")
...
def _append_pitfall(repo_root: Path, round_name: str, text: str) -> None:
    ...
    Only called when the round actually produced one."""
...
def cmd_ingest_strix(args) -> int:
    ...
    w = _db.Database(g)
    kept_urls = []
    # Asset-side seen-set. A bare upsert duplicates rows for a value that is
    # already present (re-ingesting one report twice doubles every asset).
    # A (type, value) already present is skipped instead of re-inserted, so
    # re-ingesting a strix report is idempotent.
...
def ingest_strix_findings(w, findings: list[dict],
    ...
    """Shared strix-findings ingest path.

    The CLI and the tests call THIS function, so a test
    can no longer pass by re-implementing the gate (duplicated gate logic
    stays green while the real path drifts). Gate semantics mirror
    orchestrator._ingest_finding's F15
    ...
    at 'candidate' (every row stuck, entity.transition = 0).

    Deltas vs the orchestrator path: (1) FIXED — asset-link: the shared
    ...
    for f in findings:
        ...
        if existing:
            # duplicate_of is an EDGE (F15), not an
            ...
            continue
        f.setdefault("id", util.new_id("finding"))
        # round-4 audit (HIGH: CLI ingest merges with _ingest_finding): the
        # CLI path now honors the same two policies the orchestrator uses —
        # url-less findings go to wont_test, and an in-scope finding whose
        # asset is missing mints a frontier asset so its chain can fire.
        if not f.get("url"):
            f["dedup_key"] = f.get("dedup_key") or compute_dedup_key(f)
            w.upsert_entity(f)
            w.advance_and_persist(f["id"], "wont_test", actor="validator")
            kept += 1
            continue
        ...
        # The state machine gate the docstring always
