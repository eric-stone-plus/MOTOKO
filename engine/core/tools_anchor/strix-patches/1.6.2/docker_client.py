...
def _publish_ports() -> bool:
    ''
    return os.environ.get("STRIX_SANDBOX_PUBLISH_PORTS", "").strip() == "1"
    ...
    async def _resolve_exposed_port(self, port: int) -> ExposedPortEndpoint:
        ...
        if _publish_ports():
            ...
            for proto in ("tcp", "udp"):
                bindings = (port_map.get(f"{port}/{proto}")
                            or port_map.get(port) or [])
                for binding in bindings or []:
                    host_ip = binding.get("HostIp") or "127.0.0.1"
                    host_port = binding.get("HostPort")
                    if host_port:
                        host = host_ip if host_ip not in ("", "0.0.0.0") \
                            else "127.0.0.1"
                        host = f"[{host}]" if ":" in host else host
                        return ExposedPortEndpoint(
                            host=host, port=int(host_port), tls=False)
