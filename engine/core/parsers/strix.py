...
Before this parser was upgraded, confirmed VULN blocks were dropped
entirely — only URLs and CVE strings survived, so a CRITICAL confirmed
finding never reached the graph at all.
"""
...
def _map_strix_class(title: str, cwe: str) -> str:
    """round-4 audit: Title/CWE → engine finding class, so the vuln rules'
    contains-op gates fire on the RIGHT chain (the blanket
    vuln.strix_confirmed class routed every strix finding to the same
...
class StrixParser(Parser):
    ...
    def parse(self, stdout, stderr="", action=None):
        ...
        # Pass 2: structured VULN-XXXX blocks.
