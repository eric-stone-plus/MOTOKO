...
``waf`` vendor onto the asset, which activates R-CTX-WAF-001 and the
orchestrator's per-origin cooldown. Writing the fact is what makes that rule
reachable: a rule that gates on a fact no parser stamps is dead code.
"""
