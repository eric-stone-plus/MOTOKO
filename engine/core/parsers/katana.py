...
# Resource suffixes whose query strings are version noise, never business
# parameters. Without this filter arjun can burn most of an ACT batch on
# `*.js?v=` cache-buster URLs.
_RESOURCE_SUFFIXES = (".js", ".css")

# Pure cache-buster query keys — an URL whose query contains ONLY
...
class KatanaParser(Parser):
    ...
    def parse(self, stdout, stderr="", action=None):
        ...
        for raw in (stdout or "").splitlines():
            ...
            # once per host (crawl recursion guard).
