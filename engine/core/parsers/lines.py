...
def _line_assets(stdout: str, action=None) -> list[dict]:
    ...
    for a in assets:
        ...
        # Domain-level gate — every subdomain of one registrable domain
        # must not each mint a SUB/GAU hypothesis.
...
class _LineListParser(Parser):
    def parse(self, stdout, stderr="", action=None):
        ...
        for kw in _line_assets(stdout, action):
            # _asset takes type_/value/extra, NOT arbitrary **kw — passing
            # the enum-gate keys (enumerated_host/enumerated_domain) through
            # as **kw blows up _asset() and dead-letters every run.
