...
_CLASS_MAP: list[tuple[str, str]] = [
    ...
    ("cve-", "cve"),
    ("jwt", "auth.jwt"),
    ("graphql", "fingerprint.graphql"),
    # graphiql (the browser IDE) does not contain the substring "graphql".
    ("graphiql", "fingerprint.graphql"),
...
class NucleiParser(Parser):
    ...
    def parse(self, stdout, stderr="", action=None):
        ...
        for line in stdout.splitlines():
            ...
            if cls == "auth.jwt" and isinstance(extracted, list):
                for value in extracted:
                    if isinstance(value, str) and value.strip():
                        extra["token"] = value.strip()
                        break
