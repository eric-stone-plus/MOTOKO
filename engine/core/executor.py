...
# Known tool locations on the deploy host. These are consulted BEFORE PATH
# so a host that pins its binaries is not shadowed by whatever happens to sit
# on PATH. Extend per host, or point MOTOKO_TOOLS at the toolbox.
#
# The two tree-relative entries are derived from the package location rather
# than hardcoded. A stale absolute default does not raise: resolve_tool()
# quietly falls through to PATH, surfacing as a wrong-binary or
# tool-not-found failure with no mention of the stale path.
# See util.motoko_root().
...
def _known_tool_dirs() -> tuple[Path, ...]:
    ...
    ``tools/``. ``~/.local/bin`` keeps a first-class slot because a deploy
    host may pin tool wrappers there.

    Precedence is ``~/.local/bin`` -> ``$MOTOKO_TOOLS/bin`` ->
    ``$MOTOKO_TOOLS/nuclei`` -> ``PATH`` (see ``resolve_tool``).
    """
...
# Tools whose data sources are foreign (wayback/Common Crawl for gau) and
# therefore must run through the inherited proxy. Everything else runs
# DIRECT (proxy vars stripped) — see __call__.
# The set is extensible via MOTOKO_EGRESS_PROXY_TOOLS (comma-separated tool
# names, merged with the default) so a deployment can name per-tool egress
# policy without a code change. Egress policy belongs in configuration, not
# in launch-env discipline.
_DEFAULT_PROXY_TOOLS = frozenset({"gau"})
_PROXY_TOOLS_ENV = "MOTOKO_EGRESS_PROXY_TOOLS"
_EGRESS_MODE_ENV = "MOTOKO_EGRESS_MODE"
...
def _inherits_proxy(tool: str) -> bool:
    """Whether `tool` may keep the host's proxy environment.

    `MOTOKO_EGRESS_MODE=proxy` is the anonymity-first mode: EVERY host tool
    rides the configured egress, never the host's own IP. The default keeps
    the per-tool policy below, which exposes the host IP for every tool not
    listed — prefer `proxy` for any real target.
    """
    if os.environ.get(_EGRESS_MODE_ENV, "").strip().lower() == "proxy":
        return True
    return tool in _proxy_tools()
...
class SubprocessExecutor:
    ...
    def __init__(self, writer, engagement_id: str, artifacts_dir: Path, *,
        ...
        # Live in-flight processes, {pid: run_id}. An engine that dies must
        # not leave scans running on the wire: a scan that escapes the
        # engine's process group keeps hitting the target indefinitely.
        # Registration also lands in the tool_run row (db.set_tool_run_pid)
        # for post-mortems; reap() is the shutdown path
        # (orchestrator.run finally + close()).
    ...
    def __call__(self, hyp: dict, action: dict) -> None:
        ...
        # HIGH-8 (round-3 audit): the argv[0]-basename fallback is GONE —
        # it silently executed a wrong/unrelated binary when a rule's tool
        # name drifted. A missing binary is recorded (127) and the action
        # is refused, same as any other deterministic defect.
        ...
        if runtime == "container":
            ...
            if podman is None:
                ...
                return
            # round-6 audit: a missing container is a DETERMINISTIC
            # deployment defect (tool_missing, 127) — the old path ran the
            # exec anyway and recorded a transient tool_error, feeding the
            # retry churn.
            try:
            ...
            if missing:
                ...
                self._finish_tool_run(action, status="error", exit_code=127)
                return
            # Wrap the in-container command with `timeout`
            ...
            # podman exec carries the CLIENT env into the container only via
            # explicit -e; a persistent container started under the host
            # proxy keeps those vars at STARTUP. Either way, an in-container
            # tool that must go direct must not traverse the egress proxy.
            # Non-proxy tools get the -e blanking set.
            env_clears: list[str] = []
        ...
        if env_action is not None and not isinstance(env_action, Mapping):
            ...
            return
        env = dict(os.environ)
        # Per-tool proxy policy. Tools that consume foreign archives (gau ->
        # wayback/Common Crawl) must go THROUGH the configured egress; every
        # other tool goes DIRECT. Encoding this in the launch env instead is
        # fragile: one wrong `env -u` and either gau returns 0 bytes on every
        # attempt or a direct-only tool dies. Default is DIRECT (proxy vars
        # stripped); tools in _proxy_tools() keep the inherited proxy vars.
        if not _inherits_proxy(tool):
            for k in list(env):
        ...
        # Dereference ONLY whole argv
        # tokens, ONLY from the action env (the renderer's own
        # MOTOKO_SECRET_* mapping). Substring replacement would let a
        # target-controlled value containing "@env:MOTOKO_SECRET_X" (e.g.
        # inside a URL pulled from a finding) fish the secret into a tool
        # argument — an exfiltration path to the target. Unresolved
        # references stay visible (cmd.py placeholder contract).
        _action_env = env_action or {}
        argv = [(_action_env[a[5:]]
                 if isinstance(a, str) and a.startswith("@env:")
                 and a[5:] in _action_env else a)
                for a in argv]
        try:
            with open(out_path, "wb") as out_f, open(err_path, "wb") as err_f:
                # start_new_session puts the tool in its own process
    ...
    def _register(self, run_id: str, pid: int) -> None:
        """Track a live scan (tool_run row + in-memory registry).
    ...
    def reap(self, *, grace: float = 2.0) -> list[int]:
        ...
        a setsid-escaping child re-parents into a new
    ...
    def _finish_tool_run(self, action: dict, *, status: str,
        ...
        This method ONLY closes the tool_run row. An earlier pass retired
        the hypothesis here from a
