...
providers as needed). They come from the caller, the CLI flags, or the
environment; this module has no defaults for them.
...
def reflector_from_env():
    ...
    Reads:
        ...
        MOTOKO_REFLECTOR_KEY_ENV   (env var NAME holding the key; required —
                                    there is no vendor default)
    Returns None when the model, the base_url, or the key-env name is unset
    (reflector disabled — the same fail-closed posture as a missing key).
    """
