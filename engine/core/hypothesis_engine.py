...
Rules are JSON files under ``rules/<category>/*.json`` (stdlib-only, so no
YAML dependency is needed on a headless deploy host). Matching is pure Python
...
class HypothesisEngine:
    ...
    def _load(self) -> list[dict]:
        ...
        for path in sorted(self.rules_dir.rglob("*.json")):
            ...
            # Two files can ship the same rule id (a rewrite in one category
            # next to a stale copy in another) and every asset then silently
            # gets the same baseline scan TWICE. A duplicate id is a
            # rule-authoring defect: fail loudly at load time.
            rid = rule.get("id")
            if rid:
                if rid in seen:
                    raise ValueError(
                        f"duplicate rule id {rid!r} in {path} — remove one "
                        "of the two copies before running")
                seen.add(rid)
            rules.append(rule)
        return rules
    ...
    def _eval(self, cond: dict, facts: dict) -> bool:
        ...
        if fact is None:
            ...
            # made a deep-dive rule's nested {"all": [...]} evaluate as
            # `None == None`, turning its "any" into fire-always and proposing
            # an expensive deep-dive action for every asset in the graph
