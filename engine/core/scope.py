...
A naive suffix matcher lets ``notshop.invalid`` and
...
F03: a name being in scope is NOT sufficient —
...
class ScopeGuard:
    ...
    def check_ip(self, ip: str) -> ScopeDecision:
        ...
        for n in self.in_cidrs:
            if addr in n:
                # round-4 audit: carry the literal so the replay validator
                # can pin the connection — IP-URL findings used to sit
                # inconclusive forever (no bind_ip in the decision).
