...
def engagement_dir(root: Path, engagement_id: str) -> Path:
    if not isinstance(engagement_id, str) or not re.fullmatch(
            r"[A-Za-z0-9][A-Za-z0-9_.-]{0,127}", engagement_id):
        raise ValueError("engagement id must be a single name of 1–128 ASCII characters")
    root = Path(root).resolve()
    result = root / engagement_id
    if result.resolve().parent != root:
        raise ValueError("engagement path escapes the data root")
    return result
...
def init_engagement(
    ...
    *,
    ...
    try:
        db.init_schema()
        if db.get_scope(engagement_id) is None:
            db.set_scope(
                engagement_id,
                name=name,
...
class Database:
    ...
    def __init__(self, db_path: Path | str, *, read_only: bool = False):
        ...
        if not read_only:
            ...
            try:
                ...
                os.close(fd)
                raise RuntimeError(f"another writer owns {self.path}; use read-only inspection") from None
            self._lock_fd = fd
        try:
    ...
    def tool_run_status(self, run_id: str) -> str | None:
        "A tool_run's state (schema.TOOL_RUN_STATES), or None if unknown."
        row = self.conn.execute(
            "SELECT status FROM tool_run WHERE id = ?", (run_id,)).fetchone()
    ...
    def unprocessed_observations_for_runs(self, engagement_id: str,
        ...
        return [dict(r) for r in self.conn.execute(
            f"SELECT * FROM observations WHERE engagement_id = ? "
            f"AND processed_at IS NULL AND action_id IN ({marks}) "
            f"ORDER BY created_at", (engagement_id, *run_ids))]
    ...
    def update_observation_evidence(self, obs_id: str,
                                    new_asset_ids: list[str],
                                    new_finding_ids: list[str]) -> None:
        ''
        with self.conn:
            self.conn.execute(
                "UPDATE observations SET new_asset_ids = ?, new_finding_ids = ? "
                "WHERE id = ?",
                (json.dumps(list(new_asset_ids)),
                 json.dumps(list(new_finding_ids)), obs_id))
