...
def default_root() -> Path:
    ...
    Honors ``MOTOKO_HOME``; otherwise ``<motoko_root>/runtime``, which keeps
    engine writes inside the tree by default. Set ``MOTOKO_HOME`` explicitly
    on a long-running or headless host so state survives a tree replacement.

    Layout: ``runtime/`` holds live engagement state; ``archive/`` holds
    retired engagements only.

    The root is derived from the package location (see ``util.motoko_root``),
    not hardcoded: a hardcoded absolute default does not raise when the tree
    moves — it fails silently as a missing directory.
    """
