...
Added after three field lessons:
...
* The engine had no detection feedback: an origin that started blocking
  (a CDN's deny code 60, a per-vhost 405 wall, a WAF block page) kept being
  hit at the original cadence. An origin that flips 200→405 can burn dozens
  of expansion rounds before anyone notices. Detection must cool the origin
  down, not provoke it.
* A scan that escapes the engine's process group keeps running after the
  engine dies. Knowing which scan processes are alive — and reaping them on
  exit — is an OPSEC requirement; that half lives in ``executor.py``
  (pid registry + ``reap()``).
...
The vendor list covers the common CN and global WAF products. Order matters
— first match wins, so the specific block-page titles come before the
...
_WAF_SIGNATURES: tuple[tuple[str, str], ...] = (
    ("阿里云", "aliyun-waf"),           # block-page title (functional string)
...
# Path tokens that mark a defender-planted trap (defenders do publish such
# paths in robots.txt). Matched as whole path segments split on / - _ . so a
# legit path like "bootstrap" cannot false-positive on "trap". Substring
# matching is exactly how a canary gets hit by accident.
...
class CooldownBoard:
    ...
    The clock is injectable for tests; the default is ``time.monotonic``.
    """
    ...
    def snapshot(self) -> dict:
        """M-1 (round-3 audit): serializable state for crash restarts.
        Deadlines are monotonic, so the snapshot stores REMAINING seconds
        plus a wall-clock stamp; restore() re-anchors them."""
        now = self._clock()
        return {
            "saved_at": time.time(),
            "cooldown_s": self.cooldown_s,
            "entries": [
                {"origin": o, "reason": self._reason.get(o, ""),
    ...
    def restore(self, snap: dict) -> int:
        """Re-anchor a snapshot onto this board. Returns restored count.
        Wall-clock time elapsed since the save is subtracted first."""
        if not snap:
            return 0
        now_mono = self._clock()
        elapsed = max(0.0, time.time() - float(snap.get("saved_at") or 0))
        n = 0
        for e in snap.get("entries") or []:
            rem = float(e.get("remaining_s") or 0) - elapsed
            if rem <= 0:
                continue
            o = self.normalize(e.get("origin"))
            if not o:
                continue
            self._until[o] = now_mono + rem
            self._reason[o] = str(e.get("reason") or "")
            n += 1
        return n
