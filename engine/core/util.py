...
def motoko_root() -> Path:
    ...
    Deriving rather than hardcoding keeps the tree relocatable. Relocating a
    tree that hardcodes its own absolute path silently breaks every path in
    the package (engagements root, tool search dirs), and the failure mode is
    an empty or absent directory rather than an exception — so nothing
    announces the breakage. Any new path default must go through here.
    """
    return Path(__file__).resolve().parents[2]


def default_rules_dir() -> Path:
    """Directory holding the bundled rule packs (``<category>/*.json``).

    Package-internal, so it resolves identically from a source checkout and
    from an installed wheel. An explicit ``rules_dir`` argument always wins;
