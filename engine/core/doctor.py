...
* exit 0 when nothing FAILs; WARN is informational (an operator may run
  the loop over CLI-protocol auditors with no HTTP keys, so missing keys are
...
def _check_tools() -> list[tuple[str, str]]:
    ...
    if not root.is_dir():
        out.append((WARN, f"toolbox not found: {root} (set MOTOKO_TOOLS to "
                          f"the directory holding your tool binaries)"))
    else:
        out.append((OK, f"toolbox: {root}"))
    # resolve a few well-known binaries — absence is a warning, not a
    # failure: a minimal host may only need the CLI-side auditors.
...
def _check_loop_config() -> tuple[str, str]:
    ...
    names = [a.get("name", "?") for a in cfg.get("auditors", [])]
    ...
    return OK, (f"loop config {path} valid (auditors: "
                f"{', '.join(names) or '-'}; adjudicator: {adj})")
...
def _check_egress() -> tuple[str, str]:
    """Anonymity-first check: direct egress exposes the host's own IP to
    every target and should not be used against live targets."""
    mode = os.environ.get("MOTOKO_EGRESS_MODE", "").strip().lower()
    if mode == "proxy":
        return OK, "egress mode: proxy (anonymity-first, all tools via egress)"
    if mode == "direct":
        return WARN, ("egress mode: direct — the host IP reaches targets "
                      "unrouted; prefer proxy against live targets")
    return WARN, ("egress mode: unset (defaults to direct) — set "
                  "MOTOKO_EGRESS_MODE=proxy for anonymity-first launches")
