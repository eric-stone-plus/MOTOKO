...
grok review missed-item #1: a suffix matcher lets ``notexample.com`` and
``example.com.attacker.tld`` through. This guard matches on **label
...
``example.com`` can resolve into a CIDR that is explicitly out of scope, and
...
def _domain_matches(host: str, domain: str) -> bool:
    """Label-boundary match: ``api.example.com`` matches ``example.com``,
    but ``notexample.com`` and ``example.com.evil.tld`` do NOT."""
